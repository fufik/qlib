...
class qbit:
    def __init__(self,*prms):
        if len(prms) == 2:
            ...
            if round((abs(lower)**2 + abs(upper)**2),5) == 1:
                self.vector = sp.Matrix([upper,lower])
            else:
    ...
    def __repr__(self):
        return "qbit[{},{}]".format(self.vector[0],self.vector[1])
...
def cart_prod_qbits(*ar):
    ...
    a = ar[1:]
    for l in a:
        base = sp.Matrix(sp.kronecker_product(base,l.vector))
    return base
qbit_0 = qbit(0,1)
qbit_1 = qbit(1,0)
qbit_sp = qbit_0.op_H()
